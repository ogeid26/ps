...
public class IdentifierInterpreter implements InterpreterStrategy{
    ...
    public void interpret(Node node, HashMap<String,String> types, HashMap<String,String> values, Result result) throws AssignationException, IncompatibilityException, NotDefinedException, ValueNotFoundException, EmptyContentException {
        if (types.isEmpty()){
            ...
            ExpressionNode valueName = assignNode.getExpressionNode();

            for (SubInterpreterStrategy strategy: strategies) {
                if (strategy.validate(valueName)){
                    try{
                        String message = strategy.interpret(valueName,types,values).getContent();
                        if (isString(types, nameNode, message, valueName)){
                            values.put(nameNode.getContent(), message);
                            break;
    ...
    private boolean isString(HashMap<String, String> types, IdentifierNode nameNode, String message, Node valueNode) {
        return types.get(nameNode.getContent()).equals("String") && !isInteger(message) && !(valueNode instanceof BinaryNode);
    }

    private boolean isNumber(HashMap<String, String> types, IdentifierNode nameNode, String message) {
        return types.get(nameNode.getContent()).equals("Number") && isInteger(message);
    }
